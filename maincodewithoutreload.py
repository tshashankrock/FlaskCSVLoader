        ...
        if request.form.get("link") is not None and request.form.get("link") is not '':
            ...
            if extension == ".csv":
                ...
                with open(path1,'r') as f:
                    ...
                    rowcount = len(df)
                    #run command to select id,name etc column  from table
                    string = "select \""
                    ...
                    #execute the query in backend
                    cur.execute(string)
                    print("Hello")
                    #store all the data in result in form of matrix
                    result = [list(r) for r in cur.fetchall()]
                    #store all the column value including index
